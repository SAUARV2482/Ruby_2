require 'date'
birthdate = DateTime.new(2000,2,2, 0,00,0)
    puts birthdate
...
month= (remaining_days_between/30)%12
days=  (remaining_days_between % 30.44)  
hour = (today.hour()-birthdate.hour()).abs
min = (today.min()-birthdate.min()).abs

# 

puts "#{year} year  #{month} month #{days.to_i} days #{hour} hours #{min} minitues"